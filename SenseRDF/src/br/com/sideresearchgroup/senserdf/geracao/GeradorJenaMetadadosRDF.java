 ...
 *
 */
public class GeradorJenaMetadadosRDF implements GeradorRDF {
	...
	public void gerarRDF(Dataset dataset) throws SenseRDFException {
		...
		for (Metadado metadado : dataset.getMetadados()) {
			retornarVocabularioMetadado(metadado);
			
			Property propriedade = modeloRDF.createProperty(vocabularioAtual.getNamespace() + metadado.getConstrutor());
			modeloRDF.add(nodeRaiz, propriedade, "");
		}
